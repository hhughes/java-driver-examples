...
MVN_ARGS="-Dargs.astraSecureConnectBundle='/secure-connect-bundle.zip' -Dargs.astraToken='${AT}' -Dargs.keyspace='${KEYSPACE}' -Dargs.iterations='-1'"
...
docker run --cpus=1 --rm java-driver-examples:1 /bin/bash -c "mvn -DskipTests clean package exec:java@single-region ${MVN_ARGS}"